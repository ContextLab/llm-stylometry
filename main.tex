...
Stylometry is the quantitative analysis of writing style. The field of
stylometry is broadly concerned with capturing the statistical properties of
text that characterize an author's writing style. Stylometry generally elides
the complex collection of factors that may go into an author's ``voice,''
focusing instead on word-usage statistics derived from the
text~\citep{NealEtal17}. Many mark the birth of the subject with the late
nineteenth century work of the philologist Wincenty Lutos{\l}awski, who had an
interest in finding a statistical basis for addressing a long-standing problem
in Classics of estimating the temporal order of Plato's
Dialogues~\citep{Howl91}. Toward this end, Lutos{\l}awski measured hundreds of
variables to arrive at his conclusions~\citep{Luto97}. Stylometric methods have
been used at scale to understand the evolution of style in
literature~\citep{HughEtal12} as well as judicial writing\citep{Carl16},
while also contributing to debates around uncertain
authorship~\citep{MostWall63,MostWall84,NiloBino03,Juol08}.

Here we introduce a new stylometric approach, {\em Predictive comparison
testing} (PCT), based on LLMs. PCT derives from the hypothesis that training an
LLM using the works of a single given author will produce a model that best
captures that author's unique writing style. Given two LLMs, $M_A$ and $M_B$,
trained on the works of two different authors, $A$ and $B$, respectively, we
therefore hypothesize that a held out work by Author $A$ will exhibit smaller
predictive loss when tested with model $M_A$ than with model $M_B$. Our
results, using a small data set of publicly available work from Project
Gutenberg, bear out this hypothesis by showing that we can reliably predict the
authorship of held-out text using the predictive losses of models trained on
the works of different candidate authors. We also consider a well-known example
of questioned attribution of the 15\textsuperscript{th} book in the {\emph{Oz}}
series. Our approach agrees with the current prevailing scholarly opinion about
that book~\citep{NiloBino03}, further validating our approach.
...
model to compute the prediction loss on some held-out texts from both the
target author and each of the other authors in the dataset. By comparing these
losses, we assess whether the model captures author-specific stylistic
patterns: a model trained on a given author should exhibit lower loss when
predicting that author's own texts compared to those of others.
...
one contiguous sub-sequence from each book in their training corpus (i.e., remaining books
after holding out a to-be-evaluated book). The length of the sub-sequence sampled from book~$i$
...
8 attention heads per layer. We used a training batch size of 16 and fit each
model using the AdamW optimizer~\citep{LoshHutt19} with a learning rate of $5
\times 10^{-5}$ to minimize the cross-entropy loss on the training data.
...
their books). We continue training until the cross-entropy loss fell to 3.0 or
...
quality of the resulting samples.) Training to a fixed loss threshold (e.g., as
opposed to training for a fixed number of epochs) enables us to fairly compare
model performance across authors, which is a critical component of our
...
% work this in...
It is possible to fine-tune large languages to write in the ``style'' or
``voice'' of a given author~\citep[see e.g.,][]{Mikr25}. Generally, the
mathematical encoding---or ``measure"---of a writer's style falls under the
heading of {\em stylometry}. 

Comparison of prediction at
the level of sentences and using textual information is put forward in
\citep{Reza25}. Our work differs both in the scale and reliance
purely on the loss function.



These results suggest that our approach holds promise as a new technique for
machine reading approaches to text-based disciplines
~\citep{More17,More00,Holm98} and the practices of cultural
analytics~\citep{UndeEtal13}. PCT also suggests a natural approach to author
attribution in the case in which there is a finite set of possiblities: assign
the work to the author whose PCT produces the least loss. To this we consider a
well-known example of questioned attribution of the 15th book in the
{\emph{Oz}} series and scholarly opinion~\citep{NiloBino03}, further validating
our approach.
...
\begin{center}
{\Large \bfseries Appendix A: Authors, Books, Tokens \par \label{sec:appendixA}}
\vspace{1em}
\begin{tabular}{@{}ll|ll@{}}
\toprule
