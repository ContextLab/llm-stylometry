...
%DIF DEL old.tex    Thu Mar 26 16:11:26 2026
%DIF ADD main.tex   Thu Mar 26 23:45:05 2026
...
\newcommand{\mds}{{8}}
%DIF 23a23-24
\newcommand{\embeddingpurity}{{9}} %DIF > 
\newcommand{\embeddingconfusion}{{10}} %DIF > 
%DIF -------
...
\DIFaddbegin \subsection{\DIFadd{Corpus size analysis}\label{subsec:data-requirements}}

\DIFadd{To characterize the relationship between training corpus size and attribution
accuracy, we conducted an additional set of experiments in which we varied the
number of training tokens per author. We trained models using subsets of each
author's corpus ranging from 2,500 to 643,041 tokens (the full corpus), using
the same architecture, hyperparameters, and evaluation protocol described
above. We evaluated 19 training token levels: 2,500; 5,000; 10,000; 20,000;
40,000; 45,000; 50,000; 55,000; 60,000; 65,000; 70,000; 75,000; 80,000;
128,608; 160,000; 257,216; 385,825; 514,433; and 643,041 tokens. For each
token level, we trained models for all 8 authors using 10 random seeds each
(1,520 models total).
}

\DIFadd{We modeled the relationship between training tokens and attribution accuracy
using a four-parameter sigmoid function: $y = L + K / (1 + e^{-b(\log_{10} x
- m)})$, where $L$ is the lower asymptote, $K$ is the range, $b$ controls the
steepness, and $m$ is the midpoint on the $\log_{10}$ scale. We fit this model
to the mean accuracy across authors at each token level using bounded nonlinear
least squares (}\texttt{\DIFadd{scipy.optimize.curve\_fit}}\DIFadd{). We estimated the minimum
number of tokens required to achieve $\geq 95\%$ expected attribution accuracy
via the inverse sigmoid, with a 95\% confidence interval obtained by bootstrap
resampling over authors (1,000 iterations).
}

\subsection{\DIFadd{Comparisons between predictive comparison and text embedding matching}\label{subsec:embedding-comparison}}

\DIFadd{To contextualize the performance of predictive comparison relative to
alternative text representation methods, we conducted a comparison using
pre-trained text embedding models. We selected three of the current (as of this
writing) top models from the Massive Text Embedding Benchmark~\mbox{%DIFAUXCMD
\citep[MTEB;
][]{MuenEtal23} }\hskip0pt%DIFAUXCMD
leaderboard spanning a range of sizes:
}\texttt{\DIFadd{nomic-embed-text-v1.5}} \DIFadd{(137M parameters), }\texttt{\DIFadd{bge-m3}} \DIFadd{(568M
parameters), and }\texttt{\DIFadd{Qwen3-Embedding-4B}} \DIFadd{(4.0B parameters).
}

\DIFadd{For each embedding model, we chunked each book into 1,024-token windows with
128-token overlap (using the GPT-2 tokenizer for consistency with our
predictive comparison approach), embedded each chunk, and performed
leave-one-out classification. For each held-out book, every chunk was assigned
a predicted author according to its nearest neighbor (via cosine similarity)
among all chunks from the remaining books, from all authors. We then assigned
the held-out book a predicted author according to the majority vote across its
chunk-level predictions. }\DIFaddend \begin{figure*}[t] \centering
...
\DIFaddbegin \DIFadd{Having established that our models capture author-specific stylometric
patterns, we next sought to characterize the data requirements for reliable
authorship attribution. We systematically varied the number of training tokens
per author from 2,500 to 643,041 to characterize the data requirements for
reliable authorship attribution (Fig.~\ref{fig:ntokens}A; see
Section~\ref{subsec:data-requirements}). We found that, for the range of token
counts tested, attribution accuracy followed a roughly sigmoidal trajectory on
the $\log_{10}$-token scale. A four-parameter sigmoid fit to the mean accuracy
across authors yielded $R^2 = 0.979$, with a lower asymptote of $L = 69.8\%$,
range $K = 30.9\%$, steepness $b = 5.43$, and midpoint at $10^{4.43} \approx
27{,}000$ tokens. Inverting the fitted sigmoid, we estimated that $\geq 95\%$
expected attribution accuracy requires approximately 51,000 tokens per author
(bootstrap 95\% CI: 12,000--70,000 tokens). At the lowest token count tested
(2,500 tokens), accuracy was $66.2\%$; this rose to $100\%$ at 80,000 tokens
and above (Fig.~\ref{fig:ntokens}A). Individual authors showed substantial
variability: some authors (e.g., Austen, Baum) were reliably attributed even at
low token counts, while others (e.g., Melville, Twain) required considerably
more data (Fig.~\ref{fig:ntokens}A). These per-author differences were also
reflected in the $t$-statistics at the final training epoch, which tended to
increase with training corpus size for all authors (Fig.~\ref{fig:ntokens}B).
}

\begin{figure*}[t] \centering
\includegraphics[width=\textwidth]{figs/n_tokens.pdf}
\caption{\textbf{\DIFaddFL{Attribution accuracy vs. corpus size.}} \textbf{\DIFaddFL{A.}} \DIFaddFL{Attribution
accuracy as a function of training tokens per author. Small colored dots show
per-author accuracy (averaged over 10 seeds); black squares show the mean
across all authors for each token count. The black curve is a fitted sigmoid
($R^2 = 0.979$); the gray ribbon denotes the bootstrap-estimated 95\%
confidence interval (resampled over authors). The dotted vertical line marks
the estimated minimum tokens required to achieve $\geq 95\%$ expected accuracy
($51,070$ tokens). }\textbf{\DIFaddFL{B.}} \DIFaddFL{Final-epoch (epoch 500) $t$-statistics comparing
same-author versus other-author cross-entropy losses, plotted as a function of
training tokens. Each curve denotes one author; ribbons show bootstrap 95\%
confidence intervals (resampled over seeds). The dashed horizontal line marks
the $p < 0.001$ significance threshold. The dotted vertical line marks the same
token threshold as in Panel A.}} \label{fig:ntokens} 
\end{figure*}


\DIFadd{We also wondered how the performance of our predictive comparison approach
might compare to alternative methods based on pre-trained text embeddings. For
example, since GPT-2 implicitly learn to embed each author's text, perhaps more
recent embedding models, which are trained on much larger corpora and have more
parameters, might be able to capture author-specific stylistic patterns without
the need for training separate models for each author. To investigate this
question, we compared our main predictive comparison approach to a
nearest-neighbor classification method that leverages pre-trained text
embeddings (Section~\ref{subsec:embedding-comparison}). In brief, we embedded
overlapping 1024-token chunks from each book using three top-performing
embedding models from the MTEB leaderboard~\mbox{%DIFAUXCMD
\citep{MuenEtal23}}\hskip0pt%DIFAUXCMD
. We then held out
one book at a time and assigned it a predicted author according to the majority
vote across its chunk-level nearest neighbor predictions. The results of this
comparison are shown in Figure~\ref{fig:embedding-comparison}. Book-level
attribution accuracy was 81.0\% for }\texttt{\DIFadd{nomic-embed-text-v1.5}} \DIFadd{(137M
parameters), 76.2\% for }\texttt{\DIFadd{bge-m3}} \DIFadd{(568M parameters), and 70.2\% for
}\texttt{\DIFadd{Qwen3-Embedding-4B}} \DIFadd{(4.0B parameters)---all well below the 100\%
achieved by our approach using models with just 0.8M parameters each. We also
found that accuracy exhibited an inverse relationship with model size,
indicating that general-purpose embedding quality alone (as measured by the
MTEB benchmark) does not appear to predict performance on this specialized
task. All three models correctly attributed the most stylistically distinctive
authors (Austen, Baum, and Dickens) at 100\%, but struggled with less
distinctive authors---particularly the largest model, which failed entirely on
Fitzgerald (0\% accuracy) and achieved only 38.5\% accuracy on Thompson's
books. Per-author accuracy, chunk-level purity distributions (Supp.
Fig.~\embeddingpurity), and cross-author confusion patterns (Supp.
Fig.~\embeddingconfusion) are provided in the Supplementary Materials.
}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.7\columnwidth]{figs/source/embedding_comparison.pdf}
  \caption{\textbf{\DIFaddFL{Comparison of embedding-based and predictive comparison
  approaches.}} \DIFaddFL{Book-level attribution accuracy for three pre-trained text
  embedding models from the MTEB leaderboard, compared with our predictive
  comparison approach (rightmost bar). Embedding models are ordered by size
  (137M to 4.0B parameters). Despite being orders of magnitude larger than our
  author-specific models (0.8M parameters each), none of the embedding models
  matched the 100\% accuracy of predictive comparison. Accuracy exhibited an
  inverse relationship with model size.}}
...
\DIFaddbegin \DIFadd{A critical methodological distinction separates the two approaches.
\mbox{%DIFAUXCMD
\citet{HuanEtal25} }\hskip0pt%DIFAUXCMD
fine-tune a pre-trained GPT-2 model for each author,
...
text. Although \mbox{%DIFAUXCMD
\citet{HuanEtal25} }\hskip0pt%DIFAUXCMD
argue that their training data is unlikely to
...
the only text the model has ever seen is the author's corpus. This distinction
is particularly important for forensic applications where the provenance of
model knowledge must be established.
}

\DIFadd{The data requirements of training from scratch also differ fundamentally from
fine-tuning. Our dataset-size analysis (Fig.~\ref{fig:ntokens}A) shows that
approximately 51,000 tokens per author are needed for $\geq 95\%$ expected
attribution accuracy when training from scratch. This is substantially more
than the text lengths used in standard authorship attribution benchmarks: for
example, Blogs50 averages 112 tokens per text, CCAT50 averages 506 tokens, and
Guardian averages 1,052 tokens~\mbox{%DIFAUXCMD
\citep{HuanEtal25}}\hskip0pt%DIFAUXCMD
. These benchmarks were
designed for methods that leverage pre-trained representations and thus require
far less author-specific data. The data requirements of our approach reflect a
fundamentally different regime---one in which the model must learn
author-specific patterns entirely from the author's own text. In this way, our
approach has costs and benefits relative to that of \mbox{%DIFAUXCMD
\citet{HuanEtal25}}\hskip0pt%DIFAUXCMD
.
Training from scratch requires more data and computational resources, limiting
scalability to large author datasets. However, training from scratch also
provides stronger guarantees that the model's knowledge is derived solely from
the author's text. Our approach also enabled us to perform ablation studies to
investigate the contributions of different aspects of writing style, along with
a systematic analysis of the data requirements for reliable attribution. These
would not have been possible using fine-tuned models that had already learned
general linguistic patterns during pre-training. Maintaining a pure training
corpus enables us to make stronger claims about specifically what the models
are learning.
}

\DIFaddend The information-theoretic foundations of our approach connect to earlier work
...
Our \DIFaddbegin \DIFadd{comparison with pre-trained text embedding models further highlights the
value of author-specific training. Even the largest embedding model tested
(4.0B parameters) fell well short of the 100\% accuracy achieved by our much
smaller author-specific models (0.8M parameters each), and the observed inverse
relationship between embedding model size and accuracy suggests that larger
models may encode richer content representations that actually
}\textit{\DIFadd{increase}} \DIFadd{cross-author similarity at the expense of stylistic
distinctions. The most common misclassification pattern involved attributing
texts to Dickens---the author with the largest corpus and thus the most chunks
in the nearest-neighbor pool---particularly for authors whose works overlap
thematically (e.g., Melville and Wells). By contrast, author-specific language
models trained from scratch learn to predict an individual author's sequential
word choices, capturing the patterns that distinguish one author from another
regardless of thematic overlap.
}

\DIFadd{Our }\DIFaddend reliance on books as training data contrasts with most contemporary
...
Cross-domain robustness represents another significant challenge.
\DIFdelbegin \DIFdel{Prior work
has shown that language model-based authorship attribution methods can struggle
}\DIFdelend \DIFaddbegin \DIFadd{Authorship attribution performance is known to degrade }\DIFaddend when training and test
texts \DIFdelbegin \DIFdel{come from different genres or topics~\mbox{%DIFAUXCMD
\citep{BarlStam20}}\hskip0pt%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{differ in topic or genre~\mbox{%DIFAUXCMD
\citep{Stam09, BarlStam20}}\hskip0pt%DIFAUXCMD
, and separating
stylistic signal from topical content remains an active area of
research~\mbox{%DIFAUXCMD
\citep{Stam18, FincBosc24}}\hskip0pt%DIFAUXCMD
. \mbox{%DIFAUXCMD
\citet{Stam18} }\hskip0pt%DIFAUXCMD
demonstrated that masking
topic-related words can substantially improve cross-topic attribution, and
\mbox{%DIFAUXCMD
\citet{FincBosc24} }\hskip0pt%DIFAUXCMD
showed that targeted data selection strategies are needed to
effectively leverage multi-genre training data---standard approaches that
simply pool data across genres show minimal improvement. }\DIFaddend Our experiments used
books from the same \DIFdelbegin \DIFdel{genre }\DIFdelend \DIFaddbegin \DIFadd{general period and language }\DIFaddend for each author, \DIFdelbegin \DIFdel{leaving cross-domain performance unexplored}\DIFdelend \DIFaddbegin \DIFadd{but did not
systematically control for genre or topic}\DIFaddend . The strong performance on Baum and
Thompson's \emph{Oz} books versus their non-\emph{Oz} works provides
encouraging \DIFdelbegin \DIFdel{evidence, but systematic evaluationacross diverse
domains is needed}\DIFdelend \DIFaddbegin \DIFadd{preliminary evidence that predictive comparison can generalize
across within-author topical variation, but this was not a controlled
evaluation. Systematic assessment of cross-genre and cross-register robustness
(e.g., comparing formal literary works to informal correspondence by the same
author) is an important direction for future work, particularly for
applications where the genre of the disputed text may differ from the training
corpus}\DIFaddend .
