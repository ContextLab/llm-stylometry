...
% TODO: Update after embedding results are available
...
\nameref{subsec:embedding-comparison}). Using leave-one-out evaluation on the
same 84-book corpus, book-level attribution accuracy was 81.0\% for
\texttt{nomic-embed-text-v1.5} (137M parameters), 76.2\% for \texttt{bge-m3}
(568M parameters), and \texttt{[PLACEHOLDER]}\% for
\texttt{Qwen3-Embedding-4B} (4.0B parameters)---compared to 100\% for our
predictive comparison approach using models with just 0.8M parameters each
(see Appendix for detailed results including chunk-level purity and
cross-author confusion patterns).
% NOTE: entire interpretation below assumes Qwen3-4B also underperforms 100%; verify and update after results
These results indicate that even
state-of-the-art text embedding models, despite being orders of magnitude
larger than our author-specific models, do not fully capture the stylistic
patterns that emerge from training a language model on an individual author's
works. % VERIFY: "do not fully capture" depends on Qwen result
...
% NOTE: "at most 81%" assumes Qwen3-4B also underperforms; verify after results
...
approach using models with just 0.8M parameters each. This gap suggests that
generic text representations, even from state-of-the-art models, conflate
content similarity with stylistic similarity: authors writing about similar
topics (e.g., Dickens and Melville, both Victorian-era novelists) may appear
similar in embedding space despite having distinct writing styles. In contrast,
author-specific language models trained from scratch learn to predict an
individual author's sequential word choices, capturing precisely those patterns
