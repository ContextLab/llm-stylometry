...
\subsection{Training data requirements\label{subsec:data-requirements}}
...
\subsection{Comparison with text embedding methods\label{subsec:embedding-comparison}}

To contextualize the performance of predictive comparison relative to existing
text representation methods, we conducted a comparison using pre-trained text
embedding models. We selected three models from the Massive Text Embedding
Benchmark~\citep[MTEB; ][]{MuenEtal23} leaderboard spanning a range of sizes:
...
128-token overlap (using the GPT-2 tokenizer for consistency), embedded each
chunk, and performed leave-one-out classification. For each held-out book,
every chunk was classified by its nearest neighbor (cosine similarity) among
all chunks from the remaining books. The book-level author prediction was
determined by majority vote across chunk-level predictions. We report
book-level accuracy, chunk-level purity (the fraction of chunks assigned to the
modal author), and cross-author confusion patterns.
...
We systematically varied the number of training tokens per author from 2,500
to 643,041 to characterize the data requirements for reliable authorship
attribution (Fig.~\ref{fig:ntokens}A). Attribution accuracy followed a
sigmoidal trajectory on the $\log_{10}$-token scale. A four-parameter sigmoid
fit to the mean accuracy across authors yielded $R^2 = 0.979$, with a lower
asymptote of $L = 69.8\%$, range $K = 30.9\%$, steepness $b = 5.43$, and
midpoint at $10^{4.43} \approx 27{,}000$ tokens. Inverting the fitted sigmoid,
we estimated that $\geq 95\%$ expected attribution accuracy requires
approximately 51,000 tokens per author (bootstrap 95\% CI: 12,000--70,000
tokens). At the lowest token count tested (2,500 tokens), accuracy was
$66.2\%$, rising to $100\%$ at 80,000 tokens and above
(Fig.~\ref{fig:ntokens}A). Individual authors showed substantial variability:
some authors (e.g., Austen, Baum) were reliably attributed even at low token
counts, while others (e.g., Melville, Twain) required considerably more data
(Fig.~\ref{fig:ntokens}A). These per-author differences were also reflected in
the $t$-statistics at the final training epoch, which increased monotonically
with training corpus size for all authors (Fig.~\ref{fig:ntokens}B).

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figs/n_tokens.pdf}
  \caption{\textbf{Training data requirements for authorship attribution.}
  \textbf{A.} Attribution accuracy as a function of training tokens per
  author. Small colored dots show per-author accuracy (averaged over 10
  seeds); black squares show the mean across all authors. The black curve is
  a fitted sigmoid ($R^2 = 0.979$); the gray ribbon denotes the
  bootstrap-estimated 95\% confidence interval (resampled over authors). The
  dotted vertical line marks the estimated minimum tokens for $\geq 95\%$
  expected accuracy ($\approx 51{,}000$ tokens). \textbf{B.}
  Final-epoch (epoch 500) $t$-statistics comparing same-author versus
  other-author cross-entropy losses, plotted as a function of training
  tokens. Each curve denotes one author; ribbons show bootstrap 95\%
  confidence intervals (resampled over seeds). The dashed horizontal line
  marks the $p < 0.001$ significance threshold. The dotted vertical line
  marks the same token threshold as in Panel A.}
  \label{fig:ntokens}
\end{figure*}

\subsection{Comparison with text embedding methods}

We compared predictive comparison against nearest-neighbor classification using
pre-trained text embeddings (Section~\ref{subsec:embedding-comparison}).
Book-level attribution accuracy was 81.0\% for \texttt{nomic-embed-text-v1.5}
(137M parameters), 76.2\% for \texttt{bge-m3} (568M parameters), and 70.2\%
for \texttt{Qwen3-Embedding-4B} (4.0B parameters)---well below the 100\%
achieved by our approach using models with just 0.8M parameters each
(Fig.~\ref{fig:embedding-comparison}). Strikingly, accuracy exhibited an
inverse relationship with model size, indicating that general-purpose embedding
quality (as measured by the MTEB benchmark) does not predict performance on
this specialized task. All three models correctly attributed the
most stylistically distinctive authors (Austen, Baum, and Dickens) at 100\%,
but struggled with less distinctive authors---particularly the largest model,
which failed entirely on Fitzgerald (0/8) and achieved only 38.5\% on
Thompson. Per-author accuracy, chunk-level purity distributions (Supp.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\columnwidth]{figs/source/embedding_comparison.pdf}
...
example, Blogs50 averages 112 tokens per text, CCAT50 averages 506 tokens,
and Guardian averages 1,052 tokens~\citep{HuanEtal25}. These benchmarks were
...
author-specific patterns entirely from the author's own text.
