...
To evaluate the value of training author-specific language models relative to
using generic text representations, we compared our approach against
nearest-neighbor classification using pre-trained text embeddings (see
Section~\ref{subsec:embedding-comparison}). Using leave-one-out evaluation on
the same 84-book corpus, book-level attribution accuracy was 81.0\% for
\texttt{nomic-embed-text-v1.5} (137M parameters), 76.2\% for \texttt{bge-m3}
(568M parameters), and 70.2\% for \texttt{Qwen3-Embedding-4B} (4.0B
parameters)---compared to 100\% for our predictive comparison approach using
models with just 0.8M parameters each (Supplementary Table~1).
Strikingly, accuracy exhibited an inverse relationship with model size: the
smallest embedding model outperformed the largest by over 10 percentage points.
All three models correctly attributed the works of the most stylistically
distinctive authors (Austen, Baum, and Dickens) at 100\%, but struggled with
authors whose styles are less distinctive or whose works overlap thematically
with other authors in the corpus. The largest model (\texttt{Qwen3-Embedding-4B})
failed entirely on Fitzgerald (0/8 books correct) and achieved only 38.5\% on
Thompson, suggesting that larger embedding spaces may amplify content-level
similarities at the expense of stylistic distinctions. Detailed chunk-level
purity distributions and cross-author confusion patterns are provided in
...
value of author-specific training. Despite using models with up to 4 billion
parameters, embedding-based nearest-neighbor classification achieved at most
81\% book-level accuracy on our corpus---well below the 100\% achieved by our
approach using models with just 0.8M parameters each. Moreover, accuracy
exhibited an inverse relationship with embedding model size (81.0\%, 76.2\%,
and 70.2\% for 137M, 568M, and 4.0B parameters, respectively), suggesting
that larger models may encode richer content representations that actually
\textit{increase} cross-author similarity, making stylistic distinctions harder
to detect. This is consistent with the observation that the most common
misclassification pattern involved attributing texts to Dickens---the author
with the largest corpus and thus the most chunks in the nearest-neighbor
pool---particularly for authors whose works overlap thematically (e.g.,
Melville and Wells). In contrast, author-specific language models trained from
scratch learn to predict an individual author's sequential word choices,
capturing precisely those patterns that distinguish one author from another,
