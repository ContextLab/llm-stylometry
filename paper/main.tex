...
\newcommand{\mds}{{8}}
\newcommand{\embeddingpurity}{{9}}
\newcommand{\embeddingconfusion}{{10}}
...
\textit{Oz} series, originally attributed to L. F. Baum.
...
\subsection{Corpus size analysis\label{subsec:data-requirements}}

To characterize the relationship between training corpus size and attribution
...
We modeled the relationship between training tokens and attribution accuracy
...
least squares (\texttt{scipy.optimize.curve\_fit}). We estimated the minimum
...
\subsection{Comparisons between predictive comparison and text embedding matching\label{subsec:embedding-comparison}}

To contextualize the performance of predictive comparison relative to
...
writing) top models from the Massive Text Embedding Benchmark~\citep[MTEB;
][]{MuenEtal23} leaderboard spanning a range of sizes:
\texttt{nomic-embed-text-v1.5} (137M parameters), \texttt{bge-m3} (568M
parameters), and \texttt{Qwen3-Embedding-4B} (4.0B parameters).

For each embedding model, we chunked each book into 1,024-token windows with
...
chunk-level predictions. \begin{figure*}[t] \centering
...
Having established that our models capture author-specific stylometric
...
\caption{\textbf{Attribution accuracy vs. corpus size.} \textbf{A.} Attribution
...
($51,070$ tokens). \textbf{B.} Final-epoch (epoch 500) $t$-statistics comparing
...
We also wondered how the performance of our predictive comparison approach
...
embedding models from the MTEB leaderboard~\citep{MuenEtal23}. We then held out
...
attribution accuracy was 81.0\% for \texttt{nomic-embed-text-v1.5} (137M
parameters), 76.2\% for \texttt{bge-m3} (568M parameters), and 70.2\% for
\texttt{Qwen3-Embedding-4B} (4.0B parameters)---all well below the 100\%
...
\begin{figure}[t]
  ...
  \caption{\textbf{Comparison of embedding-based and predictive comparison
  approaches.} Book-level attribution accuracy for three pre-trained text
...
A critical methodological distinction separates the two approaches.
\citet{HuanEtal25} fine-tune a pre-trained GPT-2 model for each author,
...
text. Although \citet{HuanEtal25} argue that their training data is unlikely to
...
The data requirements of training from scratch also differ fundamentally from
...
Guardian averages 1,052 tokens~\citep{HuanEtal25}. These benchmarks were
...
approach has costs and benefits relative to that of \citet{HuanEtal25}.
...
Our comparison with pre-trained text embedding models further highlights the
...
\textit{increase} cross-author similarity at the expense of stylistic
...
Authorship attribution performance is known to degrade when training and test
texts differ in topic or genre~\citep{Stam09, BarlStam20}, and separating
stylistic signal from topical content remains an active area of
research~\citep{Stam18, FincBosc24}. \citet{Stam18} demonstrated that masking
topic-related words can substantially improve cross-topic attribution, and
\citet{FincBosc24} showed that targeted data selection strategies are needed to
effectively leverage multi-genre training data---standard approaches that
simply pool data across genres show minimal improvement. Our experiments used
books from the same general period and language for each author, but did not
systematically control for genre or topic. The strong performance on Baum and
Thompson's \emph{Oz} books versus their non-\emph{Oz} works provides
encouraging preliminary evidence that predictive comparison can generalize
