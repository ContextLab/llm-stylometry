...
%% ============================================================
%% Embedding Comparison Appendix
%% ============================================================

\subsection*{Embedding-based authorship attribution comparison}

To contextualize the performance of our predictive comparison approach, we
conducted a comparison using pre-trained text embedding models from the Massive
Text Embedding Benchmark~(MTEB) leaderboard. We selected three models spanning
a range of sizes: \texttt{nomic-embed-text-v1.5} (137M parameters),
\texttt{bge-m3} (568M parameters), and \texttt{Qwen3-Embedding-4B} (4.0
billion parameters).

For each model, we chunked each book into 1,024-token windows with 128-token
overlap using the GPT-2 tokenizer (yielding 12,164 total chunks across 84
books). For each held-out book, every chunk was classified by its nearest
neighbor (cosine similarity) among all chunks from the remaining 83 books. The
book-level author prediction was determined by majority vote across chunk-level
predictions.

% All results finalized: nomic 81.0%, bge-m3 76.2%, Qwen3-4B 70.2%
\begin{table}[h]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Parameters} & \textbf{Book accuracy} & \textbf{Avg.\ purity} \\
\midrule
nomic-embed-text-v1.5 & 137M & 81.0\% (68/84) & 0.666 \\
bge-m3 & 568M & 76.2\% (64/84) & 0.694 \\
Qwen3-Embedding-4B & 4.0B & 70.2\% (59/84) & 0.703 \\
\midrule
Predictive comparison (ours) & 0.8M $\times$ 8 & 100\% (84/84) & --- \\
\bottomrule
\end{tabular}
\caption{\textbf{Book-level attribution accuracy for embedding-based
nearest-neighbor classification versus our predictive comparison approach.}
Purity denotes the fraction of held-out chunks assigned to the modal
...
We report per-author accuracy and chunk-level purity (the fraction of a
held-out book's chunks assigned to the winning author) in
Supplementary Figures~\ref{fig:embedding-purity}
and~\ref{fig:embedding-confusion}.

% VERIFIED: All 3 models complete. Findings confirmed.
Notable patterns included: (1) authors with the most distinctive styles
(Austen, Baum, Dickens) were reliably attributed by all three models at 100\%
accuracy, whereas authors with fewer books or less distinctive styles (Twain,
Melville, Fitzgerald) were frequently misclassified; (2) the largest model
(\texttt{Qwen3-Embedding-4B}, 4.0B parameters) performed worst overall at
70.2\%, completely failing on Fitzgerald (0/8 books correct) and achieving only
38.5\% on Thompson---suggesting that larger embedding models may conflate
content similarity with stylistic similarity more than smaller ones; (3) the
smallest model (\texttt{nomic-embed-text-v1.5}, 137M parameters) performed
best at 81.0\%, indicating that general MTEB ranking does not predict
performance on this specialized task; and (4) an inverse relationship between
model size and attribution accuracy was observed (81.0\%, 76.2\%, 70.2\% for
137M, 568M, and 4.0B parameters, respectively).

\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{figs/source/embedding_purity.pdf}
  \caption{\textbf{Chunk-level purity for embedding-based attribution.}
...
\begin{figure}[h]
  ...
  \caption{\textbf{Confusion matrices for embedding-based attribution.}
