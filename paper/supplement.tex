...
Qwen3-Embedding-4B & 4.0B & \texttt{[PLACEHOLDER]} & \texttt{[PLACEHOLDER]} \\
\midrule
...
% TODO: Update interpretation after Qwen3-4B results are in.
% All observations below are verified for nomic and bge-m3 only.
% After Qwen completes: (a) fill table above, (b) check if patterns hold for 3rd model,
% (c) update "outperformed" claim if Qwen changes the ranking.
Notable patterns included: (1) authors with the most distinctive styles
(Austen, Baum, Dickens) were reliably attributed by all models, whereas
authors with fewer books or less distinctive styles (Twain, Melville) were
frequently misclassified; (2) the \texttt{bge-m3} model showed a strong bias
toward classifying chunks as Dickens, likely because Dickens had the most books
(14) and thus the most chunks in the training pool; and (3) the smaller
\texttt{nomic-embed-text-v1.5} model outperformed the larger \texttt{bge-m3},
suggesting that general MTEB ranking does not directly predict performance on
