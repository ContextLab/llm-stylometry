...
def test_data_loading():
    ...
    if not expected_variants.issubset(variants):
        print(
            f"✗ Missing expected variants. Expected {expected_variants}, got {variants}"
        )
        return False

    if not baseline_present:
        print(
            f"✗ No baseline indicator found. Expected 'None' or 'nan', got {variants}"
        )
        return False
    ...
    print(
        f"  - Content models: {len(df[df['variant']=='content']['model_name'].unique())}"
    )
    print(
        f"  - Function models: {len(df[df['variant']=='function']['model_name'].unique())}"
    )
