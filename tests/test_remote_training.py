...
class TestRemoteTraining:
    ...
    def run_remote_train(self, flags="", timeout=60, kill_after_seconds=None):
        ...
        try:
            if kill_after_seconds:
                ...
                self.ssh_client(
                    "pkill -f 'python.*generate_figures.py.*--train' || true"
                )
                self.ssh_client(
    ...
    def test_variant_flag_content_only(self):
        ...
        check_result = self.ssh_client(
            "cat /tmp/llm_train.sh 2>/dev/null || echo 'FILE_NOT_FOUND'"
        )
        ...
        assert (
            "VARIANT_ARG='-co'" in check_result.stdout
        ), f"VARIANT_ARG not set correctly. Script content:\n{check_result.stdout}"

        # Check training log exists and contains variant info
        log_check = self.ssh_client(
            "ls -t ~/llm-stylometry/logs/training_*.log 2>/dev/null | head -1 | xargs cat"
        )
        ...
        if log_check.returncode == 0:
            ...
            assert (
                "-co" in log_content or "content" in log_content.lower()
    ...
    def test_all_variant_flags(self, flag, variant_name):
        ...
        assert (
            f"VARIANT_ARG='{flag}'" in check_result.stdout
    ...
    def test_baseline_no_variant(self):
        ...
        assert (
            "VARIANT_ARG=''" in check_result.stdout
    ...
    def test_resume_with_variant(self):
        ...
        assert check_result.returncode == 0
        assert (
            "RESUME_MODE='true'" in check_result.stdout
    ...
    def test_kill_mode(self):
        ...
        assert (
            "llm_training" in screen_check.stdout
        ...
        check_result = self.ssh_client("cat /tmp/llm_train.sh")
        assert (
            "VARIANT_ARG='-fo'" in check_result.stdout
    ...
    def test_repo_already_exists(self):
        ...
        if clone_check.returncode != 0:
            # Clone it first
            self.ssh_client(
                "cd ~ && git clone https://github.com/ContextLab/llm-stylometry.git"
            )
            time.sleep(5)
    ...
    def test_screen_session_created_with_correct_name(self):
        ...
        assert (
            screen_check.returncode == 0 or "llm_training" in screen_check.stdout
    ...
    def test_no_literal_dollar_signs_in_script(self):
        ...
        for line in lines:
            if line.startswith("VARIANT_ARG="):
                ...
                assert (
                    "VARIANT_ARG='-fo'" in line
                ), f"VARIANT_ARG line not expanded correctly: {line}"
                assert (
                    "$VARIANT_ARG" not in line.split("=", 1)[1]
