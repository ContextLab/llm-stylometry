...
class TestExistingDataIntegrity:
    ...
    def test_existing_baseline_data_integrity(self):
        ...
        if Path("models").exists():
            ...
            assert (
                new_duplicates == 1
            ).all(), "Duplicates still present after re-consolidation"
    ...
    @pytest.mark.skip(
        reason="Variant analyses not yet complete - re-enable once training finishes"
    )
    def test_existing_variant_data_integrity(self):
        ...
        for variant in variants:
            ...
            epoch_0_per_model = (
                df[df["epochs_completed"] == 0]
                .groupby(["seed", "train_author"])["loss_dataset"]
                .count()
            )
            ...
            if len(anomalies) > 0:
                ...
                for (seed, author), count in anomalies.head(10).items():
                    print(
                        f"  seed={seed}, author={author}, epoch_0_rows={count} "
            ...
            if models_variant_dir.exists():
                ...
                assert (
                    new_duplicates == 1
                ).all(), f"Duplicates still present in {variant} after re-consolidation"

                # Check epoch 0 counts
                new_epoch_0_per_model = (
                    df_new[df_new["epochs_completed"] == 0]
                    .groupby(["seed", "train_author"])["loss_dataset"]
                    .count()
                )
                assert (
                    new_epoch_0_per_model == expected_epoch_0_rows
                ).all(), f"Spurious epoch 0 entries still present in {variant}"
    ...
    def test_baseline_vs_variant_structure(self):
        ...
        if len(baseline_df) > 0:
            ...
            print(
                f"Variant models in baseline file: {variant_models['model_name'].nunique()}"
            )

            # Baseline file should not contain variant models
            assert (
                len(variant_models) == 0 or baseline_models["model_name"].nunique() > 0
        ...
        for variant in ["content", "function", "pos"]:
            ...
            if len(variant_df) > 0:
                ...
                assert (
                    len(unique_variants) == 1 and unique_variants[0] == variant
    ...
    def test_data_completeness(self):
        ...
        epochs_per_model = (
            ...
            .groupby("model_name")["epochs_completed"]
            .max()
        )
