...
class TestEdgeCases:
    ...
    def test_only_epoch_0(self, temp_test_dir):
        ...
        assert (
            raw_df["epochs_completed"] == 0
        ).all(), "Test data should only have epoch 0"
        ...
        assert (
            len(df[df["epochs_completed"] == 0]) == 11
        ), "Should keep all 11 epoch 0 entries"
        assert (
            len(df[df["epochs_completed"] > 0]) == 0
    ...
    def test_multiple_resume_cycles(self, temp_test_dir):
        ...
        for epoch in [10, 20, 30]:
            epoch_counts = (
                df[df["epochs_completed"] == epoch].groupby("loss_dataset").size()
            )
            assert (
                epoch_counts == 1
            ).all(), f"Duplicate epoch {epoch} not removed properly"
            assert (
                len(epoch_counts) == 12
        ...
        train_epochs = sorted(
            df[df["loss_dataset"] == "train"]["epochs_completed"].unique()
        )
        expected_epochs = list(range(1, 36))  # 1 to 35
        assert (
            train_epochs == expected_epochs
    ...
    def test_single_epoch_training(self, temp_test_dir):
        ...
        model_dir = create_test_model_directory(
            base_dir=temp_test_dir, author="fitzgerald", seed=8, max_epochs=1
        )
    ...
    def test_very_long_training(self, temp_test_dir):
        ...
        assert (
            train_epochs == expected_epochs
        ), "Epoch sequence incomplete for long training"
        ...
        epoch_250_counts = (
            df[df["epochs_completed"] == 250].groupby("loss_dataset").size()
        )
    ...
    def test_variant_models(self, temp_test_dir):
        ...
        df_content = consolidate_model_results(
            models_dir=str(temp_test_dir), variant="content"
        )

        # Check: only content models included
        assert df_content["variant"].nunique() == 1, "Should have only one variant"
        assert (
            df_content["variant"] == "content"
        ).all(), "Should only have content variant"

        # Check: deduplication worked
        assert (
            len(df_content[df_content["epochs_completed"] == 0]) == 11
        ), "Content variant should have 11 epoch 0 entries"
        epoch_27_counts = (
            df_content[df_content["epochs_completed"] == 27]
            .groupby("loss_dataset")
            .size()
        )
        assert (
            epoch_27_counts == 1
        ).all(), "Duplicate epoch 27 not removed in content variant"

        # Run consolidation for function variant
        df_function = consolidate_model_results(
            models_dir=str(temp_test_dir), variant="function"
        )

        # Check: only function models included
        assert (
            df_function["variant"] == "function"
        ).all(), "Should only have function variant"

        # Check: deduplication worked
        assert (
            len(df_function[df_function["epochs_completed"] == 0]) == 11
        ), "Function variant should have 11 epoch 0 entries"
        epoch_25_counts = (
            df_function[df_function["epochs_completed"] == 25]
            .groupby("loss_dataset")
            .size()
        )
        assert (
            epoch_25_counts == 1
        ).all(), "Duplicate epoch 25 not removed in function variant"
