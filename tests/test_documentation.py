...
def test_readme_help_commands():
    ...
    assert (
        result.returncode == 0
    ), f"run_llm_stylometry.sh --help failed: {result.stderr}"
    ...
    result = subprocess.run(
        ["./run_stats.sh", "--help"], capture_output=True, text=True, timeout=10
    )
    ...
    assert (
        "--content-only" in result.stdout
    ), "Missing --content-only flag in remote_train.sh output"
...
def test_figure_generation_with_variants():
    ...
    for variant, expected_filename in variants:
        ...
        assert (
            result.returncode == 0
        ), f"Figure generation failed for {variant}: {result.stderr}"
    ...
    for variant in variants:
        ...
        if result.returncode != 0:
            ...
            if "Insufficient" in result.stderr or "Insufficient" in result.stdout:
                ...
                pytest.fail(
                    f"Stats computation failed unexpectedly for {variant}: {result.stderr}"
                )

        # If succeeded, verify output has expected sections
        output = result.stdout + result.stderr
        assert (
            "Twain" in output or "Average" in output
        ), f"Missing expected output for {variant}"
...
def test_python_api_variant_parameter():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        fig = generate_all_losses_figure(
            data_path=str(test_data), output_path=output_path, variant=None
        )
        ...
        try:
            ...
            if fig is not None:
                ...
                assert Path(
                    expected
                ).exists(), f"Variant figure not saved at {expected}"
...
def test_remote_train_variant_flags_documented():
    ...
    assert (
        "--content-only" in result.stdout
    ), "Missing --content-only in remote_train.sh"
    assert (
        "--function-only" in result.stdout
    ), "Missing --function-only in remote_train.sh"
    assert (
        "--part-of-speech" in result.stdout
    ), "Missing --part-of-speech in remote_train.sh"
    ...
    assert (
        "Usage:" in result.stdout or "Options:" in result.stdout
