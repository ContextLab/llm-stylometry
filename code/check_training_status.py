...
def get_model_status(model_dir):
    ...
    try:
        ...
        train_loss_row = df[
            (df["epochs_completed"] == max_epoch) & (df["loss_dataset"] == "train")
        ]
...
def analyze_training_status(models_dir="models"):
    ...
    variant_models = defaultdict(
        lambda: defaultdict(list)
    )  # variant -> author -> list of (seed, status)
...
def print_variant_status(author_models, variant_name):
    ...
    for author in sorted(author_models.keys()):
        ...
        in_progress = [
            (seed, status) for seed, status in models if not status["is_complete"]
        ]
        ...
        if complete:
            ...
            if final_losses:
                ...
                print(
                    f"  Final training loss: {mean_loss:.4f} ± {std_loss:.4f} (mean ± std)"
                )
            else:
                print(f"  Completed: {len(complete)}/10 seeds (no loss data)")
        else:
            print("  Completed: 0/10 seeds")
        ...
        if in_progress:
            ...
            for seed, status in sorted(in_progress):
                ...
                if epoch > 0:
                    ...
                    print(
                        f"    Seed {seed}: epoch {epoch}/500 ({progress_pct:.1f}%) | "
    ...
    total_complete = sum(
        len([s for s, st in models if st["is_complete"]])
        for models in author_models.values()
    )
    total_in_progress = sum(
        len([s for s, st in models if not st["is_complete"]])
        for models in author_models.values()
    )

    print("\n" + "-" * 80)
    print(
        f"Summary: {total_complete}/{total_expected} complete, "
    ...
    if total_in_progress > 0:
        ...
        if all_etas:
            ...
            print(
                f"Estimated completion: {format_timedelta(max_eta)} (longest), "
