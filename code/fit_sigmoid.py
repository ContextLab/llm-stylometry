...
def load_per_author_accuracy(data_path="data/model_results_ntokens.parquet"):
    ...
    per_author = predictions.groupby(
        ['n_train_tokens', 'train_author']
...
def bootstrap_sigmoid_ci(per_author_df, n_bootstrap=1000, seed=42):
    ...
    for _ in range(n_bootstrap):
        ...
        sampled_rows = [per_author_df[per_author_df['author'] == a] for a in sampled_authors]
...
def generate_accuracy_sigmoid_figure(
    ...
    # Print results
    print(f"Sigmoid fit: y = {L:.1f} + {K:.1f} / (1 + exp(-{b:.2f} * (log10(x) - {m:.2f})))")
    ...
    palette = dict(zip(author_order, sns.color_palette("tab10", n_colors=len(author_order))))
    ...
    for author in author_order:
        ...
        ax.scatter(
            jittered_tokens, author_data['accuracy'].values,
            color=palette[author], s=15, alpha=0.4, zorder=4,
    ...
    ax.scatter(
        mean_sorted['n_tokens'].values, mean_sorted['accuracy'].values,
        color='black', s=40, zorder=5, marker='s', alpha=0.6,
    ...
    if threshold_tokens_95:
        ax.axvline(
            x=threshold_tokens_95, color='gray', linestyle=':',
    ...
    fit_results = {
        "L": float(L), "K": float(K), "b": float(b), "m": float(m),
        "r_squared": float(r_squared),
        "threshold_tokens_95": float(threshold_tokens_95) if threshold_tokens_95 else None,
